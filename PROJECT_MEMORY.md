...
- For GCP-to-MSU image promotion, use the verified server-to-server path as the default: log into MSU, then have MSU SSH directly to GCP 1 with `~/.ssh/id_gcp` and stream `docker save` into MSU `docker load`. This was reverified on 2026-08-02 against GCP 1 `34.42.83.50` and returned the expected target image ID. Do not relay multi-gigabyte images through the development workstation, and do not substitute an MSU-local rebuild when exact image-ID parity is required.
