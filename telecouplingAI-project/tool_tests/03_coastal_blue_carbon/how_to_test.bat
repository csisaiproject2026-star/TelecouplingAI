        @echo off
        REM ================================================================
        REM  Tool: Coastal Blue Carbon
        REM  Tool call name: run_coastal_blue_carbon
        REM  Category: InVEST (natcap.invest)
        REM ================================================================
        REM
        REM HOW TO TEST
        REM -----------
        REM Option A — via the web UI (recommended):
        REM   1. Start the platform:  cd .. && docker compose up -d
        REM   2. Open http://localhost in your browser
        REM   3. Upload files from the testdata/ folder
        REM   4. Send a chat message like:
        REM      "Prompt: run Coastal Blue Carbon main model, snapshots=snapshots.csv, transitions=transitions_sample.csv, biophysical=biophysical_table_sample.csv"
        REM   5. Outputs will appear in output/ after the run completes
        REM
        REM Option B — standalone Python (non-InVEST tools only):
        REM   conda activate TeleCouplingAI
        REM   cd ..\..\backend
        REM   python -c "
        REM     import asyncio, sys, os
        REM     sys.path.insert(0, os.getcwd())
        REM     from tools.blue_carbon import *
        REM     # See SKILL.md for exact parameter format
        REM   "
        REM
        REM INPUT FILES
        REM -----------
        REM Upload: snapshots.csv, transitions_sample.csv, biophysical_table_sample.csv, GBJC_*.tif (x3)
Data path: ../datainput_for_demo/CoastalBlueCarbon_input/
Prompt: run Coastal Blue Carbon main model, snapshots=snapshots.csv, transitions=transitions_sample.csv, biophysical=biophysical_table_sample.csv
        REM
        REM OUTPUT FILES go to: output/
        REM ================================================================
        echo Test guide for: Coastal Blue Carbon
        echo.
        echo See comments above for test instructions.
        echo Input files are in testdata/
        echo Outputs will be written to output/
        pause
