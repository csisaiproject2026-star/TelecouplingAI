...
@pytest.mark.parametrize(
    ("message", "expected"),
    [
        ("Start a different tourism workflow from scratch.", True),
        ("Create another soybean pipeline.", True),
        ("Start a new telecoupling analysis.", True),
        ("从头开始一个新的旅游工作流。", True),
        ("开始一个新的旅游分析。", True),
        ("Run the existing workflow with new files.", False),
        ("Use a different model in this analysis.", False),
        ("Summarize the analysis; do not replace the workflow.", False),
        ("请总结分析，不要替换当前工作流。", False),
    ],
)
def test_workflow_replacement_requires_explicit_replacement_intent(message, expected):
    assert agent._looks_like_workflow_replacement(message) is expected
...
@pytest.mark.parametrize(
    "message",
    [
        "Analyze a new tourism telecoupling workflow.",
        "Start a different tourism workflow from scratch.\n[[FILES_ATTACHED]]",
    ],
)
async def test_replacement_plan_includes_files_attached_with_current_request(
    monkeypatch,
    message,
):
    manager = _ScopeSessionManager()
    current_file = {
        "filename": "new-flows.csv",
        "path": "/uploads/new-flows.csv",
        "current_batch": True,
    }
    manager.uploaded.append({
    ...
    planning_text = model_inputs[0][0].parts[0].text
    assert manager.plan["_workflow_scope"]["upload_start_index"] == 1
    assert current_file["filename"] in planning_text
    assert current_file["path"] in planning_text
    assert "old.csv" not in planning_text
    assert "old-result.csv" not in planning_text
    assert "old soybean objective" not in planning_text
...
@pytest.mark.parametrize(
    "message",
    [
        "Run the telecoupling analysis now.",
        "Run the analysis now.",
    ],
)
async def test_run_intent_does_not_replace_existing_telecoupling_plan(
    monkeypatch,
    message,
):
    manager = _ScopeSessionManager()
    ...
    async def fake_generate(client, model, contents, config, event_callback):
        ...
        content = types.Content(
            role="model",
            parts=[types.Part.from_text(text="Running the saved plan.")],
        )
        return SimpleNamespace(candidates=[SimpleNamespace(content=content)])
    ...
    assert allowed == ["execute_workflow_plan"]
    assert manager.plan_writes == []


@pytest.mark.asyncio
async def test_summary_request_does_not_replace_existing_telecoupling_plan(monkeypatch):
    manager = _ScopeSessionManager()

    async def fake_generate(client, model, contents, config, event_callback):
        content = types.Content(
            role="model",
            parts=[types.Part.from_text(text="Here is the current workflow summary.")],
        )
        return SimpleNamespace(candidates=[SimpleNamespace(content=content)])
    ...
    await agent.run_agent(
        "Summarize the current telecoupling workflow.",
        "workflow-summary",
        [],
        lambda event: None,
        session_manager=manager,
    ...
    assert manager.plan_writes == []
    assert manager.plan["case_name"] == "old"
