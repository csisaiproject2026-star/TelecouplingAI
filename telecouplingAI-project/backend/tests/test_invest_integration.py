...
class TestRecreationIntegration:
    def test_recreation_server_reachable_and_run(self, tmp_path):
        ...
        a proxy or firewall on a local machine). Always runs on GCP.
        """
        import socket
        import natcap.invest.recreation.recmodel_client as rec
        # quick TCP probe — skip if port 54321 is not reachable within 5 s
        try:
...
class TestCoastalVulnerabilityIntegration:
    def test_coastal_vulnerability_run(self, tmp_path):
        ...
        cv_dir = sample("CoastalVulnerability")
        args = {
            "workspace_dir":             str(tmp_path),
            "results_suffix":            "",
...
class TestOffshoreWindEnergyIntegration:
    def test_wind_energy_run(self, tmp_path):
        ...
        we_dir = sample("WindEnergy")
        base_dir = sample("Base_Data")
        args = {
            "workspace_dir":              str(tmp_path),
            "results_suffix":             "",
            "wind_data_path":             os.path.join(we_dir, "input", "ECNA_EEZ_WEBPAR_Aug27_2012.csv"),
            "aoi_vector_path":            os.path.join(we_dir, "input", "New_England_US_Aoi.shp"),
            "bathymetry_path":            os.path.join(base_dir, "global_dem.tif"),
            "land_polygon_vector_path":   os.path.join(base_dir, "global_polygon.shp"),
            "turbine_parameters_path":    os.path.join(we_dir, "input", "3_6_turbine.csv"),
            "number_of_turbines":         80,
            "global_wind_parameters_path": os.path.join(we_dir, "input", "global_wind_energy_parameters.csv"),
            "min_depth":                  3,
            "max_depth":                  60,
            "min_distance":               0,
            "max_distance":               200000,
...
class TestCBCPreprocessorIntegration:
    ...
    def test_cbc_preprocessor_run(self, tmp_path):
        ...
        cbc_dir = sample("CoastalBlueCarbon")
        lookup_patched = self._patch_code_column(
            os.path.join(cbc_dir, "inputs", "lulc_lookup.csv"), tmp_path, "lulc_lookup_p.csv"
        )
        args = {
            "workspace_dir":          str(tmp_path),
            "results_suffix":         "",
            "landcover_snapshot_csv": os.path.join(cbc_dir, "inputs", "snapshots.csv"),
...
class TestCBCMainIntegration:
    ...
    def test_cbc_main_run(self, tmp_path):
        ...
        bio_patched = self._patch_code_column(
            os.path.join(cbc_dir, "outputs_preprocessor", "biophysical_table_sample.csv"),
        ...
        args = {
            ...
            "results_suffix":              "",
            "landcover_snapshot_csv":      os.path.join(cbc_dir, "inputs", "snapshots.csv"),
            "landcover_transitions_table": os.path.join(cbc_dir, "outputs_preprocessor",
...
class TestCropProductionPercentileIntegration:
    ...
    def test_crop_percentile_run(self, tmp_path):
        ...
        crop_dir = sample("CropProduction")
        args = {
            "workspace_dir":               str(tmp_path),
            "results_suffix":              "",
...
class TestSeasonalWaterYieldIntegration:
    def test_swy_run(self, tmp_path):
        ...
        swy_dir = sample("Seasonal_Water_Yield")
        args = {
            "workspace_dir":               str(tmp_path),
            "results_suffix":              "",
            ...
            "biophysical_table_path":      os.path.join(swy_dir,
...
class TestForestCarbonEdgeEffectIntegration:
    def test_forest_carbon_edge_run(self, tmp_path):
        ...
        fce_dir = sample("forest_carbon_edge_effect")
        args = {
            "workspace_dir":                                str(tmp_path),
            "results_suffix":                               "",
...
class TestScenicQualityIntegration:
    def test_scenic_quality_run(self, tmp_path):
        ...
        sq_dir = sample("ScenicQuality")
        args = {
            "workspace_dir":  str(tmp_path),
            "results_suffix": "",
            "aoi_path":       os.path.join(sq_dir, "Input", "AOI_WCVI.shp"),
            "structure_path": os.path.join(sq_dir, "Input", "AquaWEM_points.shp"),
            "dem_path":       os.path.join(sq_dir, "Input", "claybark_dem.tif"),
            "refraction":     0.13,
            "do_valuation":   False,
        }
        sq.execute(args)
...
class TestHRAIntegration:
    def test_hra_run(self, tmp_path):
        ...
        hra_dir = sample("HabitatRiskAssess")
        args = {
            "workspace_dir":          str(tmp_path),
            "results_suffix":         "",
            "aoi_vector_path":        os.path.join(hra_dir, "Input", "subregions.shp"),
            "info_table_path":        os.path.join(hra_dir, "Input",
                                                    "habitat_stressor_info.csv"),
            "criteria_table_path":    os.path.join(hra_dir, "Input",
        ...
        try:
            ...
            # Summary statistics step can fail with sample data geometry;
            # RISK TIFs are written before that step and are the key output.
            pass
        all_files = []
        for root, _, files in os.walk(str(tmp_path)):
            all_files.extend(files)
...
class TestWaveEnergyIntegration:
    def test_wave_energy_run(self, tmp_path):
        ...
        we_dir = sample("WaveEnergy")
        base_dir = sample("Base_Data")
        args = {
            "workspace_dir":       str(tmp_path),
            "results_suffix":      "",
            "wave_base_data_path": os.path.join(we_dir, "input", "WaveData"),
            "analysis_area":       "westcoast",
            "aoi_path":            os.path.join(we_dir, "input", "AOI_WCVI.shp"),
            "machine_perf_path":   os.path.join(we_dir, "input",
                                                "Machine_AquaBuOY_Performance.csv"),
            "machine_param_path":  os.path.join(we_dir, "input",
                                                "Machine_AquaBuOY_Parameter.csv"),
            "dem_path":            os.path.join(base_dir, "global_dem.tif"),
            "valuation_container": False,
        }
        we.execute(args)
        all_files = []
...
class TestUrbanCoolingIntegration:
    def test_urban_cooling_run(self, tmp_path):
        ...
        uc_dir = sample("UrbanCoolingModel")
        args = {
            "workspace_dir":             str(tmp_path),
            "results_suffix":            "",
...
class TestUrbanFloodIntegration:
    def test_urban_flood_run(self, tmp_path):
        ...
        uf_dir = sample("UrbanFloodMitigation")
        args = {
            "workspace_dir":                      str(tmp_path),
            "results_suffix":                     "",
            ...
            "curve_number_table_path":            os.path.join(uf_dir,
...
class TestUrbanNatureAccessIntegration:
    def test_urban_nature_access_run(self, tmp_path):
        ...
        una_dir = sample("UrbanNatureAccess")
        args = {
            "workspace_dir":                 str(tmp_path),
            "results_suffix":                "",
            ...
            "admin_boundaries_vector_path":  os.path.join(una_dir,
                                                          "administrative-units.shp"),
...
class TestScenarioGenProximityIntegration:
    def test_scenario_proximity_run(self, tmp_path):
        ...
        sp_dir = sample("scenario_proximity")
        args = {
            "workspace_dir":            str(tmp_path),
            "results_suffix":           "",
